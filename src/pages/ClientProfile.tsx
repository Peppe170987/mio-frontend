...
export default function ClientProfile() {
  ...
  const fetchProfileData = async () => {
    ...
    if (!error && data) {
      setFormData(prev => ({
        ...
        email: user.email || '',
        password: '' // Reset della password field per sicurezza
      }));
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Aggiorna i metadati del profilo (Nome e Cognome)
      ...
      // 2. Aggiorna Email (se cambiata)
      if (formData.email !== user.email) {
        const { error: emailError } = await supabase.auth.updateUser({
          email: formData.email
        });
        if (emailError) throw emailError;
        alert("Richiesta cambio email inviata! Conferma tramite il link ricevuto.");
      }

      // 3. Aggiorna Password (se inserita)
      ...
      // AGGIORNAMENTO DATI LOCALI: Ricarichiamo i dati per mostrare le modifiche
      await fetchProfileData();
      
      alert("Profilo aggiornato con successo!");
      // Opzionale: navigate('/client-dashboard'); // Scommenta se vuoi tornare indietro in automatico
    } catch (error: any) {
      ...
      <nav className="fixed top-0 left-0 right-0 bg-white shadow-sm z-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <Link to="/client-dashboard" className="p-2 text-gray-400 hover:text-orange-600 transition-colors">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-6 mb-10">
              ...
              <div>
                ...
                <p className="text-orange-600 font-bold text-xs uppercase italic mt-1">Area personale Atleta</p>
              </div>
            </div>
